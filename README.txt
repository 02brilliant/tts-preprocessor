...
- 3·4단계 entrypoint는 각각 `bin/build_standard_llm_entrypoint.py`,
  `bin/build_natural_llm_entrypoint.py`이며 원문에 2단계 전체 규칙을 한 번 적용한
  뒤 호출 gate가 필요성을 확인하고 단계별 고정 프롬프트로 LLM을 호출한다.
  gate가 명백히 불필요하다고 판단하면 규칙 결과를 최종 출력으로 사용한다.
- 3·4단계는 규칙 모듈이 단계별 축약·복합명사 경계·운율·잔여 읽기의 유한 후보를
  만들고 LLM은 전체 문맥으로 후보 ID와 option만 선택한다. 최종 문자열은
  코드가 조합한다. 4단계는 여기에 전용 표준발음 확정 규칙과 문맥 후보를 더한다.
- 웹서버/API는 `TTS_PREPROCESSOR_BINARY`로 지정한
  `packages/tts-preprocessor/tts-preprocessor-standard`를 호출한다.
...
- `build/macos/dist/tts-preprocessor-standard`
- `build/macos/dist/tts-preprocessor-simplified`
- `build/macos/dist/tts-preprocessor-standard-llm`
- `build/macos/dist/tts-preprocessor-natural-llm`
- `downloads/tts-preprocessor-macos.zip`

ZIP 루트에는 네 실행 파일(1·2·3·4단계)과 `README.txt`만 포함된다. 현재 빌드는

...
| `contract_violation` | LLM 단계 계약을 위반해 출력에 삽입·교체된 문자 |
| `contract_violation_deleted` | Speech 입력에서 삭제된 공백·줄바꿈·쉼표·고정 문장부호 |
...
| LLM 계약 위반 | `.diff-contract-violation` | 주황 배경·굵은 테두리 |
| 삭제된 Speech 구조 | `.diff-del.diff-contract-violation-deleted` | 주황 테두리·빨간 취소선 |

색상만으로 단계를 판별하지 않도록 결과 위에 1·2·3단계, 삭제, LLM 계약 위반
범례를 항상 표시한다.

Prosody 계약 위반 시 모델 원출력은 숨기지 않는다. 출력 원문 영역에서는
허용되지 않은 삽입·교체 문자를 주황 테두리로 표시하고, 인접 Diff에서는 원문에서
삭제된 문자를 기존 빨간 취소선으로 함께 표시한다. 오류 결과는 진단용으로만
표시하며 3단계 입력으로 사용하지 않는다.

Speech 계약 위반 시에도 모델 원출력을 출력 원문 영역에 표시한다. 발음 철자의
허용된 변환은 3단계 보라색으로 유지하고, 출력에서 바뀌거나 추가된 공백·줄바꿈·
쉼표·고정 문장부호만 주황 테두리로 구분한다. 입력에서 사라진 구조 문자는 인접
Diff에 주황 테두리와 빨간 취소선으로 표시한다. 오류 결과는 최종 출력으로
채택하지 않으며 1·2단계 결과를 유지한다.

## 5. 공백과 개행

- 단계에서 새로 생성된 ASCII 공백은 `␣`로 표시한다.
- 삭제된 공백도 `␣`로 표시한다.
- 계약을 위반해 추가된 탭·CR·개행은 각각 `⇥`, `␍`, `↵`로 표시한다.

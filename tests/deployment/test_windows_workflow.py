...
def test_windows_workflow_builds_smokes_and_uploads_one_flat_zip() -> None:
    ...
    assert "tts_preprocessor_llm_standard.spec" in workflow
    ...
    assert "build_llm_standard_entrypoint" in workflow
    ...
    assert "dist\\tts-preprocessor-llm-standard.exe" in workflow
    assert "Smoke test executable" in workflow
    assert "Extracted Windows executable smoke test failed" in workflow
    assert '$expected = @("README.txt", "tts-preprocessor-llm-minimal.exe", "tts-preprocessor-llm-natural.exe", "tts-preprocessor-llm-standard.exe", "tts-preprocessor-simplified.exe", "tts-preprocessor.exe")' in workflow

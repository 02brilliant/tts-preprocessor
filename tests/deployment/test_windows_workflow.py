...
def test_windows_workflow_builds_smokes_and_uploads_one_flat_zip() -> None:
    ...
    assert "tts_preprocessor_standard_llm.spec" in workflow
    assert "tts_preprocessor_natural_llm.spec" in workflow
    assert "build_binary_entrypoint" in workflow
    assert "build_simplified_binary_entrypoint" in workflow
    assert "build_standard_llm_entrypoint" in workflow
    assert "build_natural_llm_entrypoint" in workflow
    assert "dist\\tts-preprocessor-standard.exe" in workflow
    assert "dist\\tts-preprocessor-simplified.exe" in workflow
    assert "dist\\tts-preprocessor-standard-llm.exe" in workflow

...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("2025년1월3일", "이천이십오년 일월 삼일"),
        ("행사는 1월 3일입니다", "행사는 일월 삼일입니다"),
        ("행사는 6월19일입니다", "행사는 유월 십구일입니다"),
        ("2025년 1월", "이천이십오년 일월"),
        ("2025년1월", "이천이십오년 일월"),
...
def test_compact_korean_date_inserts_component_spaces_and_keeps_markers() -> None:
    output = transform_with_trace("6월19일")

    assert output.normalized_text == "유월 십구일"
    assert [
        (piece.text, piece.provenance, piece.source_span, piece.owner)
        for piece in output.render_pieces
    ] == [
        ("유", "GENERATED_READING", SourceSpan(0, 1), "date"),
        ("월", "ORIGINAL_KOREAN", SourceSpan(1, 2), None),
        (" 십구", "GENERATED_READING", SourceSpan(2, 4), "date"),
        ("일", "ORIGINAL_KOREAN", SourceSpan(4, 5), None),
    ]

...
def test_spaced_slash_boundary_transforms_each_numeric_unit_segment() -> None:
    ...
    expected = (
        "무게는 각각 마이너스 일-쩜-오영-킬로그램 / 십이-쩜-오-킬로그램 / "
        "이-쩜-사구영-킬로그램 / 플러스 삼-쩜-사영-킬로그램 / "
        "플러스 삼-쩜-이일영-킬로그램 이다."
    )
...
def test_spaced_slash_boundary_preserves_raw_delimiter_spacing() -> None:
    ...
    expected = "무게는 각각 마이너스 일-쩜-오영-킬로그램  /  십이-쩜-오-킬로그램 이다."
    assert transform(text) == expected
...
@pytest.mark.parametrize(
    ("text", "expected"),
    [
        ('무게는 각각 +.5kg / 12.5kg 이다.', '무게는 각각 +.5kg / 십이-쩜-오-킬로그램 이다.'),
        ('무게는 각각 1,00kg / 12.5kg 이다.', '무게는 각각 1,00kg / 십이-쩜-오-킬로그램 이다.'),
        ('무게는 각각 +01.5kg / 12.5kg 이다.', '무게는 각각 +01.5kg / 십이-쩜-오-킬로그램 이다.'),
    ],
)
...
def test_spaced_slash_boundary_does_not_create_no_hangul_slash_list() -> None:
    text = "-1.50kg / 12.5kg"
    assert transform(text) == "-1.50kg / 십이-쩜-오-킬로그램"

...
ARTICLE_EXPECTED = """기본계획은 삼대 추진전략과 열두 개 세부 과제로 짜였다. 첫 번째 축은 에이아이 시대의 핵심 인프라인 공간정보를 다각도로 구축하는 것이다. 국토부는 에이아이 고속도로의 토대가 될 삼차원 고정밀 핵심 공간정보 육종을 구축할 계획이다. 기존 오천분의 일 축척 중심의 이차원 지도에서 벗어나 전 국토를 고정밀 삼차원 입체지도, 이른바 ‘신대동여지도’로 새롭게 구축해 디지털 전환을 이뤄내고, 천분의 일 지도 구축 범위도 넓힌다는 방침이다.

이렇게 확보한 삼차원 고정밀 공간정보는 에이아이 학습데이터로도 활용된다. 국토부는 이를 토대로 공간을 분석해 의사결정을 지원하는 ‘GeoAI 파운데이션 모델’ 개발을 추진하고, 관련 연구개발 투자도 늘리기로 했다. 위성 인프라 확충 계획도 눈에 띈다. 현재 운영 중인 국토위성 일·이호기에 더해 삼·사호기를 추가로 도입하고, 기상 상황과 무관하게 상시 관측이 가능한 레이더 위성도 새로 도입해 다양한 위성영상을 확보할 방침이다."""

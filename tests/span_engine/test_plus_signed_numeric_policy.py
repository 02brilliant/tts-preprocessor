...
def test_plus_general_number_reading_and_invalid_preserve():
    cases = [
        ("+1 올랐다", "플러스 일 올랐다"),
        ('값은 +1.5입니다', '값은 플러스 일-쩜-오입니다'),
        ('오차는 +0.05다', '오차는 플러스 영-쩜-영오다'),
        ('+1,000.50 증가', '플러스 천-쩜-오영 증가'),
    ]
...
def test_plus_numeric_delimited_owners():
    cases = [
        ("+1:2 비율", "플러스 일 대 이 비율"),
        ("1:+2 비율", "일 대 플러스 이 비율"),
        ('+1.5:-2.0 경기', '플러스 일-쩜-오 대 마이너스 이-쩜-영 경기'),
        ('+0.0:1 배율', '플러스 영-쩜-영 대 일 배율'),
        ('+1,000.50:2 스케일', '플러스 천-쩜-오영 대 이 스케일'),
        ("+1:2:3", "플러스 일 대 이 대 삼"),
        ("1:+2:-3:4", "일 대 플러스 이 대 마이너스 삼 대 사"),
        ('+1.2:2.3:+3.0', '플러스 일-쩜-이 대 이-쩜-삼 대 플러스 삼-쩜-영'),
        ("+1,000:2,000:+3,000", "플러스 천 대 이천 대 플러스 삼천"),
        ('+1.5~+2.0kg', '플러스 일-쩜-오에서 플러스 이-쩜-영-킬로그램'),
        ("-1~+2kg", "마이너스 일에서 플러스 이-킬로그램"),
        ('+0.0∼1.5cm', '플러스 영-쩜-영에서 일-쩜-오-센티미터'),
        (
            "+1,000.50〜+2,000.75원",
            "플러스 천-쩜-오영에서 플러스 이천-쩜-칠오-원",
        ),
    ]
    for source, expected in cases:
        assert transform(source) == expected
...
def test_plus_numeric_delimited_invalid_and_context_preserve():
    for source in (
        ...
        assert transform(source) == source
    assert transform("+1.5~2테스트") == '플러스 일-쩜-오에서 이 테스트'

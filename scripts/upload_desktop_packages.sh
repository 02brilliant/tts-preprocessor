...
validate_windows_archive() (
  ...
  if [[ ! -f "$extract_dir/README.txt" \
    || ! -f "$extract_dir/tts-preprocessor-standard.exe" \
    || ! -f "$extract_dir/tts-preprocessor-standard-llm.exe" \
    || ! -f "$extract_dir/tts-preprocessor-natural-llm.exe" \
    || ! -f "$extract_dir/tts-preprocessor-simplified.exe" \
    || -L "$extract_dir/README.txt" \
    || -L "$extract_dir/tts-preprocessor-standard.exe" \
    || -L "$extract_dir/tts-preprocessor-standard-llm.exe" \
    || -L "$extract_dir/tts-preprocessor-natural-llm.exe" \

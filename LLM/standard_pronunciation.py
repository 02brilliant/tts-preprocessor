...
class StandardPronunciationDataError(ValueError):
    """The packaged stage-4 pronunciation registry is missing or invalid."""
...
def _load_stage4_pronunciations(
    ...
    try:
        ...
        raise StandardPronunciationDataError(
            "4단계 표준발음 사전 파일을 찾을 수 없습니다."
        ) from exc
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise StandardPronunciationDataError(
            "4단계 표준발음 사전은 유효한 UTF-8 JSON이어야 합니다."
        ) from exc

    if not isinstance(payload, dict) or payload.get("schema_version") not in {1, 2}:
        raise StandardPronunciationDataError(
            "4단계 표준발음 사전 schema_version은 1 또는 2여야 합니다."
        )
    raw_entries = payload.get("entries")
    if not isinstance(raw_entries, list) or not raw_entries:
        raise StandardPronunciationDataError(
            "4단계 표준발음 사전에 entries가 필요합니다."
        )
    ...
    for index, raw in enumerate(raw_entries):
        if not isinstance(raw, dict):
            raise StandardPronunciationDataError(
                f"4단계 표준발음 사전 entries[{index}]가 객체가 아닙니다."
            )
        required = (
            "surface",
            "pronunciation",
        ...
        if any(
            ...
            raise StandardPronunciationDataError(
                f"4단계 표준발음 사전 entries[{index}] 필드가 유효하지 않습니다."
            )
        if raw["mode"] not in _ALLOWED_MODES:
            raise StandardPronunciationDataError(
                f"4단계 표준발음 사전 entries[{index}] mode가 유효하지 않습니다."
            )
        if raw["surface"] == raw["pronunciation"]:
            raise StandardPronunciationDataError(
                f"4단계 표준발음 사전 entries[{index}]가 원형과 같습니다."
            )
        if raw["category"] not in _ALLOWED_CATEGORIES:
            raise StandardPronunciationDataError(
                f"4단계 표준발음 사전 entries[{index}] category가 유효하지 않습니다."
            )
        if (
            ...
            raise StandardPronunciationDataError(
                f"4단계 표준발음 사전 entries[{index}] 표면은 한글과 단일 공백만 허용합니다."
            )
        if not raw["source"].startswith("https://"):
            raise StandardPronunciationDataError(
                f"4단계 표준발음 사전 entries[{index}] source는 HTTPS URL이어야 합니다."
            )
        ...
        if boundary_type not in _ALLOWED_BOUNDARY_TYPES:
            raise StandardPronunciationDataError(
                f"4단계 표준발음 사전 entries[{index}] boundary_type이 유효하지 않습니다."
            )
        if raw["mode"] == "contextual" and boundary_type != "contextual":
            raise StandardPronunciationDataError(
                f"4단계 표준발음 사전 entries[{index}] contextual 경계가 필요합니다."
            )
        ...
        if (
            ...
            raise StandardPronunciationDataError(
                f"4단계 표준발음 사전 entries[{index}] allowed_tails가 유효하지 않습니다."
            )
        if boundary_type == "fixed_phrase" and raw_tails:
            raise StandardPronunciationDataError(
                f"4단계 표준발음 사전 entries[{index}] fixed_phrase에는 allowed_tails를 둘 수 없습니다."
            )
        ...
        if paradigm_id is not None and (
            ...
            raise StandardPronunciationDataError(
                f"4단계 표준발음 사전 entries[{index}] paradigm_id가 유효하지 않습니다."
            )

        if raw["surface"] in surfaces:
            raise StandardPronunciationDataError(
                f"4단계 표준발음 사전에 중복 표면이 있습니다: {raw['surface']}"
            )

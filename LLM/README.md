...
사용하지 않는다. 단, 1단계가 ASCII 공백으로 양쪽이 분리된 `뉴스`를 `news`로
확정한 경우에는 2단계가 이를 다시 `뉴스`로 바꾸지 않도록 서버 내부에서만
잠금 토큰으로 치환하고 응답 검증 뒤 `news`로 복원한다.
...
` 뉴스 `에서 생성된 ` news `는 이 API 내부에서만 `<LOCK_0001>` 형태로
잠금 처리된다. 호출자는 잠금 목록이나 provenance를 전달하지 않으며, 응답에는
복원된 `news`만 반환된다.

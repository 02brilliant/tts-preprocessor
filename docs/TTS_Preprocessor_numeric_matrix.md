...
`지하`/`지상`, registered location nouns, or the exact stair-access movement
...
|---|---|---|---|---|---|---|---|
| unit | integer, decimal, comma integer/decimal for registered unit policy; Korean large-unit integer/decimal cores such as `300만㎡`, `3만kg`, `3.5만kg`, and `3만TB`; compact/paired large-unit ranges such as `45~50만kg` and `1억~2억kg`; Hangul-context numberless forms such as `수 km` and `한글 ㎡ 한글` for the approved registered-unit list; attached-only English `in` (`10in`) plus spaced `ft`/`min`/`TB`; finance aliases `bp`/`BP` (`베이시스 포인트`) reuse the existing numeric-prefix rule and stay Hangul-context excluded so they do not consume `bps`; bare `bit`/`bits` with Hangul tails; CJK `㎕`/`㎗`/`㎘`/`㎛`/`㎚`/`㎍`/`㎔`/`㎩`/`㎪`/`㎸`/`㎼`/`㎟`/`㎣`/`㎳`/`㎲`/`㎱`/`㎰` plus ASCII SI-prefix aliases such as `µm`/`nm`/`µg`/`uL`/`nL`/`pL`/`kV`/`Pa`/`hPa`/`GW`/`THz`/`sec`/`ms`/`µs` reuse the existing numeric-prefix rule; `DL`, `KL`, `KW`, `KV`, `GPA`, `pm`, and single-letter `s` are not unit aliases; `kHz` and `KB` inherit the same decimal eligibility; SI `m`/`M` remains case-sensitive; exact natural caret-power allowlist is limited to `mm·cm·km·m` directly followed by `^2` or `^3` | `+`/`-` plus owner-local dash-like minus aliases such as `–` only under full-claim signed unit conditions | yes for registered simple/special units | yes for valid comma integer/decimal | registered units allow attached or one ASCII space except `in`, which is attached-only; Hangul-context units insert one generated space only when the unit is attached to a left Hangul neighbor; caret unit-to-exponent has no gap | malformed numeric+unit and unsafe tails preserve; unsupported `alphabetic-unit^exponent` keeps that literal block while a separately valid numeric prefix may be read; exponent-only fallback is forbidden; unregistered English tails after large units stay literal (`3만abc`); standalone or English-only unit tokens stay literal (`km`, `the kg`, `a bit`, `10 in`); all-caps Latin units without a left Hangul neighbor keep acronym reading (`GB그룹`, `TB는`, `GW는`); `KL` is not a kiloliter alias | `2.35kHz -> 이쩜삼오 킬로헤르츠`, `2.35KB -> 이쩜삼오 킬로바이트`, `10000.5kg -> 만쩜오 킬로그램`, `2.35m^2 -> 이쩜삼오 제곱미터`, `300만㎡ -> 삼백만 제곱미터`, `3만kg -> 삼만 킬로그램`, `3.5만kg -> 삼쩜오 만 킬로그램`, `45~50만kg -> 사십오에서 오십만 킬로그램`, `1㎕ -> 일 마이크로리터`, `1㎗ -> 일 데시리터`, `1㎘ -> 일 킬로리터`, `5nm -> 오 나노미터`, `5Pa -> 오 파스칼`, `1013hPa -> 천십삼 헥토파스칼`, `1GW -> 일 기가와트`, `10in -> 십 인치`, `3min -> 삼 분`, `5sec -> 오 초`, `5ms -> 오 밀리초`, `5µs -> 오 마이크로초`, `10bp -> 십 베이시스 포인트`, `10TB -> 십 테라바이트`, `bit -> 비트`, `수 km을 달려왔다 -> 수 킬로미터를 달려왔다`, `2.35KB^2 -> 이쩜삼오KB^2` |
...
The approximate tail `께` is admitted after `분` or `초` only when the time
owner has already full-consumed `N시 N분`, `N분 N초`, or `N시 N분 N초`.
It is not a broad safe tail for bare `N분`: `오전 9시 6분께 -> 오전 아홉 시
육분께`, `5분 15초께 -> 오분 십오초께`, while `6분께` preserves.
Attached clock-hour `N시께` uses the clock-hour meaning tail and reads
`9시께 -> 아홉 시께`. Honorific-person readings such as
`6분께 -> 여섯 분께` remain out of scope.
...
Clock-hour `N시` is handled by the time owner with an owner-local safe Korean tail whitelist. Safe attached tails such as `을/를/은/는/이/가/로/으로/와/과/도/만/부터/까지/에/에는/에서/에도/보다/처럼/마다` and `이다/입니다/인/이면/면/이라면/라면/이라고/라고/인데/였다/이었다` are allowed without opening broad numeric suffix fallback. Lexical/code-like continuations such as `시스템`, `시장`, `시험`, `시즌`, and `시abc` remain residual or preserve-first. `시리즈` is a registered Sino numeric suffix with a generated space. Compact `N시방향` and spaced `N시 방향` always render with a space before `방향`; unsigned 1–12 use native clock-hour readings and all other values use Sino.
...
| compound exact unit | exact `bps`, `Kbps`, `kbps`, `Mbps`, `mbps`, `Gbps`, `gbps`, `Tbps`, `tbps`, `rpm`, `fps`, `ppm`, `ppb`, `dBi` | `+`/`-` plus owner-local minus aliases, same as simple units; `+-` remains preserve | yes | yes for valid comma integer/decimal | attached numeric prefix or one ASCII space; slash-internal spaces stay blocked | malformed numeric core, unsafe tail, URL/path/protected context preserves atomically | `10Mbps -> 십 메가비피에스`, `10 Mbps -> 십 메가비피에스`, `+10 Mbps -> 플러스 십 메가비피에스`, `10~20 Mbps -> 십에서 이십 메가비피에스`, `10bps -> 십 비피에스` |

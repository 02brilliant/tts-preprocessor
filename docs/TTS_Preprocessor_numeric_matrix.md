...
| compound exact unit | exact `Kbps`, `kbps`, `Mbps`, `mbps`, `Gbps`, `gbps`, `Tbps`, `tbps`, `rpm`, `fps`, `ppm`, `ppb`, `dBi` | unsigned; existing signed compound policy unchanged | yes | yes for valid comma integer/decimal | attached numeric prefix only | malformed numeric core, unsafe tail, URL/path/protected context preserves atomically | `2.35Mbps -> 이쩜삼오 메가비피에스`, `10Kbps -> 십 킬로비피에스`, `2Tbps -> 이 테라비피에스`, `2.35rpm -> 이쩜삼오 알피엠` |

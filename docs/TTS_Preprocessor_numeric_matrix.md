...
|---|---|---|---|---|---|---|---|
| unit | integer, decimal, comma integer/decimal for supported unit policy; owner-local meter aliases include `m` and fullwidth Latin `ｍ`; registered power units include `W`, `kW`, `MW`, `Wh`, `kWh`, `MWh`; a registered ASCII-letter unit immediately followed by `^2` or `^3` uses the caret-power owner | `+`/`-` plus owner-local dash-like minus aliases such as `–` only under full-claim signed unit conditions | yes, inheriting the base unit's decimal eligibility | yes for allowed unit matrix | registered power units allow attached or one ASCII space; other number-to-unit spacing remains owner-local; unit-to-`^2`/`^3` must have no space | malformed numeric+unit and alphabetic unsafe tails preserve without partial numeric fallback; caret power is licensed only at end-of-input or before whitespace/Hangul, and a digit/ASCII-letter tail does not use power reading | `2.5MW -> 이쩜오 메가와트`, `3MWh -> 삼 메가와트시`, `7m^2 -> 칠 제곱미터`; `ML` remains the existing milliliter alias |

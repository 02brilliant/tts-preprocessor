...
- `Mbps`/`Kbps`/`Gbps`/`Tbps`/`rpm`/`fps`/`ppm`/`ppb`/`dBi`도
  `bps`와 같이 숫자와 단위 사이 ASCII 공백 한 칸을 허용한다.
  `10 Mbps -> 십 메가비피에스`, `60 fps -> 육십 에프피에스`,
  `9 dBi -> 구 디비아이`. `+10 Mbps`, `10~20 Mbps`, `제 10 Mbps`도
  붙임형과 같다. `in` 전치사와 슬래시 내부 공백 `10 km / h`는 열지 않는다.

## Bit-rate unit `bps`

- 숫자 접두 `bps`는 exact compound unit으로 `비피에스`를 읽는다.
  기존 `Kbps`/`Mbps`/`Gbps`/`Tbps`와 같은 붙임 숫자 접두·소수 규칙이다.
  `10bps -> 십 비피에스`, `10 bps -> 십 비피에스`,
  `2.35bps -> 이쩜삼오 비피에스`. ASCII 공백 한 칸은 허용하고 미등록
  `BPS`는 열지 않는다. `bp`/`BP` 베이시스 포인트와 구분한다.
- 독립 `제` 뒤에서도 같은 단위 규칙을 따른다. `제10bps -> 제십 비피에스`,
  `제 10bps -> 제 십 비피에스`, `제 10 bps -> 제 십 비피에스`.

## Prefixed `제` is free-standing; followers split by owner

- `제`+(숫자)+(한글 단위)는 `제` 앞에 다른 글자가 없을 때만 서수 규칙이다.
  `제`와 숫자 사이 공백 유무는 같다. `제5차`/`제 5차` -> `제 오차`.
- `A제 5차`처럼 `제`가 앞 글자에 붙어 있으면 서수 규칙이 아니다. `A제`는
  남기고 뒤의 `5차`는 일반 숫자 정책으로 `오차`가 된다. 붙여 쓴 `A제5차`는
  한 토큰으로 보존한다.
- `제10kg`/`제10bp`는 `제`를 남기고 등록 Latin 단위 규칙을 따른다.
  `제십 킬로그램`, `제십 베이시스 포인트`. `제2.5문항`은 `제 이쩜오문항`,
  `제2문항abc`는 `제 이문항abc`. 단위 없는 `제2`/`제2.5`도 한자어다.
- `제5G`, `제5abc`, `제2-문항`처럼 미등록 코드형 꼬리는 보존한다.

## Basis point unit `bp`/`BP`

- 숫자 접두 `bp`/`BP`는 simple unit으로 `베이시스 포인트`를 읽는다.
  기존 등록 Latin 단위와 같이 붙임/ASCII 공백 한 칸, 소수, 부호, 한국어
  큰수 핵, 구간을 허용한다. `10bp -> 십 베이시스 포인트`,
  `2.5bp -> 이쩜오 베이시스 포인트`, `3만bp -> 삼만 베이시스 포인트`,
  `45~50만bp -> 사십오에서 오십만 베이시스 포인트`.
- 전송률 `bps`와 구분한다. `10bps`와 불완전 꼬리 `10bpabc`는 원문
  유지한다. `Bp`는 등록하지 않는다. 혈압·두문자어 충돌 때문에 한글
  문맥 `수 bp`는 단위로 읽지 않는다. 숫자 없는 대문자 `BP`는 기존
  두문자어 `비피`를 유지한다. 붙임형 분수 `1/2bp`는 `1/2kg`와 같다.

## Hangul word ending in `제` plus spaced counter

- `이제`, `어제`, `경제`, `과제`처럼 한글 단어가 `제`로 끝나는 경우는
  뒤의 숫자+단위를 일반 counter/number로 읽는다.
  `이제 10년밖에 되지 않았지만, 10년밖에 되지 않았다. ->
  이제 십년밖에 되지 않았지만, 십년밖에 되지 않았다.`
  앞 글자에 붙은 `제`는 서수가 아니다. `A제 5차 -> A제 오차`,
  `A제5차`는 붙여 쓴 한 토큰으로 보존한다.

## English second family `sec`/`ms`/`µs`

- 한 글자 `s`는 열지 않는다. 영문 초 단위는 `sec`/`Sec`/`secs`만
  numeric prefix로 `초`를 읽는다. `SEC`와 풀네임 `second(s)`는 제외한다.
- 같은 숫자 접두·소수·부호·한국어 큰수 핵·구간 규칙을 `min`과 공유한다.
  `5sec -> 오 초`, `2.5ms -> 이쩜오 밀리초`, `+2.5sec -> 플러스 이쩜오 초`,
  `3만sec -> 삼만 초`, `45~50만sec -> 사십오에서 오십만 초`.
  붙임형 분수 `1/2sec`는 `1/2kg`와 같이 단순 단위가 가져가지 않는다.
- SI 접두 별칭은 `ms`/`msec`/`㎳`, `µs`/`μs`/`us`/`µsec`/`μsec`/`usec`/
  `㎲`, `ns`/`nsec`/`㎱`, `ps`/`㎰`이다. `MS`/`US`는 두문자어로 남긴다.
- `5m/sec`와 `5m/s`의 초속 복합 단위는 그대로다. 한글 문맥은 `수 sec`/
  `수 ms`만 허용하고 `us`/`ps`/`secs`/`min`은 제외한다.

## Registered SI prefix completion for live units

- 이미 살아있는 단위 계열에 빠진 SI 접두어와 입력 별칭만 추가한다.
  길이 `µm`/`μm`/`um`/`㎛`, `nm`/`㎚`, 면적·부피 `mm²`/`mm2`/`㎟`,
  `mm³`/`mm3`/`㎣`, 질량 `µg`/`μg`/`ug`/`㎍`, `ng`, `pg`, 부피 `uL`,
  `nL`, `pL`, 주파수 `THz`/`Thz`/`thz`/`㎔`와 `KHz`/`khz`/`Mhz`/`mhz`,
  전압 `kV`/`kv`/`㎸`, `µV`/`μV`/`uV`/`㎶`, `nV`/`㎵`, 압력 `Pa`/`㎩`,
  `kPa`/`㎪`, `hPa`, `GPa`, 전력 `µW`/`μW`/`uW`/`㎼`, `kw`, `GW`, `TW`,
  에너지 `mWh`, `GWh`, `TWh`, 전송률 `Kbps`/`kbps`/`mbps`/`gbps`/
  `Tbps`/`tbps`.
- `pm`은 시각 표기 `5pm`과 겹치므로 피코미터로 등록하지 않는다.
  영문 초 계열은 이후 `sec`를 연 뒤 `ms`/`µs`를 추가했다. `KW`/`KV`/
  `GPA`/`NL`/`PL`/`ul`/`㎙`/`㎺`도 두문자어·식별자·femto/pico 충돌로
  닫아 둔다.
- 한글 문맥에서는 `um`(영어 머뭇거림), `ug`, `pg`(페이지), ASCII
  `mm2`/`mm3`를 제외한다. `수 GW`는 기가와트이고 `GW는`는 두문자어다.

## Microliter and deciliter CJK compatibility symbols `㎕` and `㎗`

- 입력 기호 `㎕`, `㎗`만 기존 special-unit 레지스트리에 추가한다.
  numeric prefix, 소수, 한국어 큰수 핵, 한글 문맥, unsafe-tail preserve는
  `㎖`/`㎘`와 같다. `1㎕ -> 일 마이크로리터`, `1㎗당 -> 일 데시리터당`.
- ASCII 대응은 `µL`/`μL`와 `dL`/`dl`이다. `DL`은 두문자어이므로 등록하지
  않는다. `mg/dL` 복합 단위는 기존 데시리터당 읽기를 유지한다.

## Deployed binary freshness for registered units

- 운영 API는 작업 트리 `engine/`이 아니라 방금 빌드한 패키지 바이너리를
  실행한다. 로컬 pytest만 통과한 미반영 HEAD 바이너리는 `1㎘당`,
  `1만㎡`, `수 km`, `지상 3층`, `3.5만kg`, `45~50만kg`를 원문으로 남긴다.
- 이 예문은 core semantic probe `registered_unit_surface.py`에 둔다.
  배포는 rsync 전에 로컬 작업 트리에서 같은 core suite를 돌리고,
  Linux dist/staging/published 바이너리와 기동 후 live API에서도 돌린다.
- 패키징 대상이 dirty이면 deploy ID에 `-dirty`를 붙이고 해당 경로를
  출력한다. 기대 결과는 배포 셸 스크립트가 아니라 probe 파일에 둔다.

## Kiloliter CJK compatibility symbol `㎘`

- 입력 기호 `㎘`만 기존 special-unit 레지스트리에 추가한다.
  numeric prefix, 소수, 한국어 큰수 핵, 한글 문맥, unsafe-tail preserve는
  `㎖`/`ℓ`와 같다. `1㎘당 -> 일 킬로리터당`.
- ASCII 대응은 `kL`/`kl`만 `킬로리터`다. `KL`은 두문자어이므로 등록하지
  않는다.

## Inch, foot, minute, terabyte, and bit units

- `in`, `ft`, `min`, `TB`는 숫자 접두와 한국어 큰수 핵이 있으면 읽는다.
  `10in -> 십 인치`, `5ft -> 오 피트`, `3min -> 삼 분`,
  `10TB -> 십 테라바이트`, `3만TB -> 삼만 테라바이트`.
  `in`만 영어 전치사 충돌 때문에 숫자와 붙여 쓴 경우만 허용한다.
  `10 in`은 숫자를 읽고 `in`은 원문 유지한다. `ft`/`min`/`TB`는 기존처럼
  ASCII 공백 한 칸을 허용한다.
- `bit`/`bits`는 숫자 접두뿐 아니라 단독 입력과 한글이 붙은 경우에도
  `비트`로 읽는다. `bit`, `bit는`, `8bit`. 영문 단어 안이나
  `a bit`/`the bit`는 원문 유지한다.
- `TB` 단독은 `GB`와 같이 원문을 유지하고, `TB는`만 기존 두문자어
  `티비는`을 유지한다. `수 TB`처럼 왼쪽 한글이 있으면 테라바이트다.

## Hangul-context registered units

- 등록 simple/special unit은 숫자 접두가 없어도, 한글 음절이 단위에
  직접 닿거나 ASCII 공백 하나를 사이에 두면 읽는다.
  `수 km을 달려왔다 -> 수 킬로미터를 달려왔다`,
  `연면적 ㎡ 규모로 조성된다. -> 연면적 제곱미터 규모로 조성된다.`
  한글에 붙은 단위 앞에는 생성 공백 한 칸을 둔다. `수km -> 수 킬로미터`.
- 단독 입력과 영문 전용 문맥은 유지한다. `km`, `the kg`, `a km away`.
- 대문자 전용 라틴 단위(`GB`, `MB`)는 왼쪽 한글이 있을 때만 단위로 읽고,
  오른쪽에만 한글이 붙은 `GB그룹`/`GB는`은 기존 두문자어 읽기를 유지한다.
- 단일 문자, 퍼센트, ASCII 지수 별칭, 온도/도수, `km/h`류 복합 단위,
  `KB`/`ML`/`MV`처럼 두문자어 충돌이 큰 단위는 숫자 접두 규칙을 유지한다.
  숫자+한글에 붙은 `21명kg`/`3가kg`도 기존 unsafe-tail preserve를 유지한다.

## Floor location prefixes `지하` and `지상`

- `층`의 위치 확정 접두에 `지상`을 `지하`와 같은 exact prefix로 추가했다.
  `지상 3층 -> 지상 삼 층`,
  `지하 1층부터 지상 3층까지 -> 지하 일 층부터 지상 삼 층까지`.
  접두 없는 `1층부터 3층까지`와 이동·개수 가능 표현은 계속 유보한다.

## Korean large-unit registered units

- 한국어 만/억 큰수 뒤에 등록 simple/special unit이 오면 규칙 엔진이
  단위까지 읽는다. `300만㎡ -> 삼백만 제곱미터`,
  `3만kg -> 삼만 킬로그램`, `3만km -> 삼만 킬로미터`.
  소수 큰수도 같다. `3.5만kg -> 삼쩜오 만 킬로그램`.
  공유 접미 구간 `45~50만kg -> 사십오에서 오십만 킬로그램`,
  양쪽 큰수 구간 `1억~2억kg -> 일억에서 이억 킬로그램`.
  미등록 영문 꼬리만 원문을 유지한다. `3만abc -> 삼만abc`.

## Prefixed ordinal, signed Sino units, and clock direction

- `제`+숫자+등록 한글 단위는 고유어/hybrid/유보 여부와 관계없이 한자어다.
  `제` 다음은 항상 한 칸이다. 숫자와 단위는 기존 제 접두 붙임형을 유지한다.
  `제3조 -> 제 삼조`, `제3 조 -> 제 삼 조`, `제4가지 -> 제 사가지`,
  `제 3가지 -> 제 삼가지`, `제3조각 -> 제 삼조각`,
  `제3시리즈 -> 제 삼시리즈`, `제3자 -> 제 삼자`.
  `조`만 입력의 숫자-단위 공백을 보존한다. 미등록 한글 꼬리도
  `제`가 있으면 숫자를 한자어로 읽는다. `제3시점 -> 제 삼시점`,
  `제3시회의 -> 제 삼시회의`.
- 한자어 단위의 부호 정수도 읽는다. `+3시리즈 -> 플러스 삼 시리즈`,
  `+1분기 -> 플러스 일분기`.
- `조각` 부호 정수는 기존 hybrid를 따른다. `1~39`는 고유어+공백,
  `40+`와 소수는 한자어+공백이다.
- `N시방향`/`N시 방향`은 부호 없는 `1~12`만 clock-hour 고유어이고
  나머지는 한자어다. 붙임형 입력도 출력은 항상 `시 방향`으로 띄운다.
  `제`가 붙은 `시`/`시방향`은 한자어 붙임형이다.
  `제3시 -> 제 삼시`, `제3시방향 -> 제 삼시방향`.
- `시리즈`는 잔여 어휘가 아니라 등록 Sino numeric suffix다.

## Residual numeric fallback

- 의미 확정에 실패한 숫자+한글 단위는 즉시 원문 종료하지 않는다.
  `분`/`가지`의 부호 없는 `1~99`만 기존 의미 유보를 유지하고, 나머지 단위는
  `40` 이상 한자어, `분`/`가지`는 `100` 이상·`0`·소수·분수·부호 있는 수를
  한자어 잔여로 읽는다. 잔여 경로만 숫자와 한글 사이 공백 한 칸을 둔다.
- 정의되지 않은 붙임 한글은 숫자 core만 읽는다.
  `3시리즈 -> 삼 시리즈`, `11시스템 -> 십일 시스템`.
  등록 시각 잔여 어휘는 `시스템·시장·시험·시즌`이다. `시리즈`는 등록
  Sino suffix다.
  `3시회의`, `1~12`의 미등록 시각 꼬리, `25시`처럼 `40` 미만 무효 시각은
  원문 유지하고 `99시 -> 구십구 시`다.
- 정수 부호는 소수와 같은 기본 signed 계약이다. `+-N`과 `±N`은
  `플러스 마이너스` + 숫자 읽기다.

## Clock-hour meaning tails

- 붙임형 clock-hour `N시`의 의미 꼬리로 `부로`, `께`, `반`, `방향`을
  추가했다. 숫자 읽기와 `시` 간격은 기존 clock-hour canonical을 유지하며
  꼬리 한글은 source-exact다. `0시부로 -> 영 시부로`,
  `1시부로 -> 한 시부로`, `9시께 -> 아홉 시께`, `1시반 -> 한 시반`,
  `3시방향 -> 세 시 방향`이다. 방향 표면은 숫자 읽기 뒤 `시`와
  `방향` 사이를 항상 한 칸 띄운다.
- clock-hour 안전 꼬리 뒤에 같은 허용 조사/어미가 이어져도 시각 surface로
  인정한다. `1시부터는 -> 한 시부터는`, `2시까지로 -> 두 시까지로`.
- 이 확장은 붙임형 `N시` hour-only 경계에만 적용한다. `6분께` preserve와
  `분`/`초`/`시간` suffix 경계, 기존 lexical/code-like preserve는 유지한다.

## Default LLM model

- 생략 시 기본 LLM을 `gemma4:31b`에서 `gemma4-31B-it (vLLM)`로 변경했다.
  명시적 model 선택이 있으면 등록된 다른 모델을 계속 사용할 수 있다.
